	...
	for (let i = 0; i < MESSAGE_COUNT; i++) {
		...
		if ((i + 1) % 100 === 0) {
			process.stdout.write(
				`\r📝 Processing: ${i + 1}/${MESSAGE_COUNT} (${(((i + 1) / MESSAGE_COUNT) * 100).toFixed(1)}%)`,
			);
		}
	}
...
async function runAllBenchmarks() {
	...
	try {
		...
		if (results.ans104 && results.httpSig) {
			const speedDiff = (
				((results.httpSig.opsPerSecond - results.ans104.opsPerSecond) / results.ans104.opsPerSecond) *
				100
			).toFixed(1);
			console.log(
				`🏆 Fastest signing method: ${results.ans104.opsPerSecond > results.httpSig.opsPerSecond ? 'ANS-104' : 'HTTP-SIG'}`,
			);
			console.log(
				`📊 Performance difference: ${Math.abs(speedDiff)}% ${speedDiff > 0 ? 'HTTP-SIG faster' : 'ANS-104 faster'}`,
			);
		}
