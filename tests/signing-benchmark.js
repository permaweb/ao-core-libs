...
async function benchmarkHTTPSIGSigning() {
  return await measurePerformance(
    'HTTP-SIG Message Signing (Local)',
    LARGE_BATCH,
    async (i) => {
      // Create message data
      const messageData = {
...
async function benchmarkLargePayloads() {
  return await measurePerformance(
    'Large Payload Handling (1KB messages)',
    50,
    async (i) => {
...
async function runSigningBenchmarks() {
  ...
  try {
    ...
    console.log(`🏃 Performance difference: ${Math.abs(perfDiff).toFixed(1)}% ${perfDiff > 0 ? '(HTTP-SIG faster)' : '(ANS-104 faster)'}`);
